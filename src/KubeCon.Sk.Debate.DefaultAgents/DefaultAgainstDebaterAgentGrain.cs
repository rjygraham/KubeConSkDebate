...
public class DefaultAgainstDebaterAgentGrain(Kernel kernel, ILogger<DefaultAgainstDebaterAgentGrain> logger) : AgentGrainBase
{
    ...
    - Debate against the topic provided by the moderator.
        ...
        await foreach (ChatMessageContent response in agent.InvokeAsync(history))
        {
            switch (response)
            {
                case OpenAIChatMessageContent aoaiChatMessage:
                    await debateGrain.AddChatMessageAsync(new Abstractions.Models.ChatMessage { Author = nameof(DefaultAgainstDebaterAgentGrain), Content = response.ToString(), Role = ChatMessageAuthorRole.Assistant }, debate.Moderator);
                    break;
                default:
                    break;
            }
